...
/**
 * @author alexandre Altere no arquivo Matrix a terceira coluna: 2 para origem e
 *         3 para destino.
 */
...
public class AStar {
	 ...
	 */
	/*
	 * origem 2 5 destino 8 5 ok origem 0 2 destino 8 5 ok origem 5 8 destino 4
	 * 3 ok origem 5 8 destino 3 2 ok origem 5 8 destino 2 2 ok origem 5 8
	 * destino 0 2 ok origem 9 0 destino 0 9 ok origem 9 0 destino 0 2 ok
	 */
	...
	public static void main(String[] args) {
		...
		// String path =
		// "/home/alexandre/Dropbox/netbeansproject/AStar/src/astar/matrixImpossible";
		String valueStart = "";
		String valueEnd = "";
		String adjTiles = "";
		...
		while (!openList.isEmpty() && !findDest) {
			...
			if (Integer.parseInt(posi[0]) != STARTROW
					&& Integer.parseInt(posi[1]) != STARTCOL) {
			...
			for (int j = 0; j < adjTilesArray.length; j++) {
				...
				if (!openList.contains(adjTilesArray[j])
						&& !closeList.contains(adjTilesArray[j])
						&& !adjTilesArray[j].equals("")) {
					openList.add(adjTilesArray[j]);
					posi = adjTilesArray[j].split(";");
					if (matrix[Integer.parseInt(posi[0])][Integer
							.parseInt(posi[1])] != 3) {
						matrix[Integer.parseInt(posi[0])][Integer
								.parseInt(posi[1])] = 5;
					} else {
						findDest = true;
						key = posi;
						posi = valueStart.split(";");
						parent.put(key[0] + " " + key[1], posi[0] + " "
								+ posi[1]);
					}
				}
	...
	public String adjPoint(int[][] matrix, String point) {
		...
		if (value[0] == 0 && value[1] == 0) {
			...
			if (matrix[value[0]][value[1] + 1] == 0
					|| matrix[value[0]][value[1] + 1] == 3) {

				step = as.countDist(AStar.STARTROW, AStar.STARTCOL, value[0],
						(value[1] + 1));
				heuristic = as.countDist(value[0], (value[1] + 1),
						AStar.ENDROW, AStar.ENDCOL);
				adj = adj + value[0] + ";" + (value[1] + 1) + ";"
						+ (step + heuristic);
				adj = adj + "/";

			}

			if (matrix[value[0] + 1][value[1] + 1] == 0
					|| matrix[value[0] + 1][value[1] + 1] == 3) {

				step = as.countDist(AStar.STARTROW, AStar.STARTCOL,
						(value[0] + 1), (value[1] + 1));
				heuristic = as.countDist((value[0] + 1), (value[1] + 1),
						AStar.ENDROW, AStar.ENDCOL);
				adj = adj + (value[0] + 1) + ";" + (value[1] + 1) + ";"
						+ (step + heuristic);
				adj = adj + "/";

			}

			if (matrix[value[0] + 1][value[1]] == 0
					|| matrix[value[0] + 1][value[1]] == 3) {

				step = as.countDist(AStar.STARTROW, AStar.STARTCOL,
						(value[0] + 1), (value[1]));
				heuristic = as.countDist((value[0] + 1), (value[1]),
						AStar.ENDROW, AStar.ENDCOL);
				adj = adj + (value[0] + 1) + ";" + (value[1]) + ";"
						+ (step + heuristic);
		...
		if (matrix[value[0] - 1][value[1]] == 0
			...
			step = as.countDist(AStar.STARTROW, AStar.STARTCOL, value[0] - 1,
					(value[1]));
			heuristic = as.countDist(value[0] - 1, (value[1]), AStar.ENDROW,
					AStar.ENDCOL);
			adj = adj + (value[0] - 1) + ";" + (value[1]) + ";"
					+ (step + heuristic);
			adj = adj + "/";
		...
		if (matrix[value[0]][value[1] + 1] == 0
			...
			step = as.countDist(AStar.STARTROW, AStar.STARTCOL, (value[0]),
					(value[1] + 1));
			heuristic = as.countDist((value[0]), (value[1] + 1), AStar.ENDROW,
					AStar.ENDCOL);
			adj = adj + (value[0]) + ";" + (value[1] + 1) + ";"
					+ (step + heuristic);
			adj = adj + "/";
	...
	public int countDist(int startPointX, int startPointY, int endPointX,
			int endPointY) {
